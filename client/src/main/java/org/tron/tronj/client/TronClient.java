...
public class TronClient {
     ...
     * @throws IllegalException if fail to unfreeze asset
     */
    public TransactionExtention accountPermissionUpdate(String ownerAddress, String owner_permission,  String[] active_permissions) throws InvalidProtocolBufferException {
        AccountPermissionUpdateContract contract =
                createAccountPermissionContract(parseAddress(ownerAddress), owner_permission, active_permissions);
    ...
    public static byte addressPreFixByte = (byte) 0x41;;

    public static String addressPreFixString = "41";
    ...
    public static boolean addressValid(byte[] address) {
        ...
        if (address[0] != addressPreFixByte) {
            System.out.println("Warning: Address need prefix with " + addressPreFixByte + " but "
                    + address[0] + " !!");
            return false;
        }
        return true;
    ...
    public AccountPermissionUpdateContract createAccountPermissionContract(
            ByteString owner, String owner_permission, String[] active_permissions) throws InvalidProtocolBufferException {
        ...
        try {
            ...
            parser.merge(owner_permission, builder1);
            ownerPermission = builder1.build();

            for(Common.Key key : ownerPermission.getKeysList()){
                addressValid(key.getAddress().toByteArray());
            }
            /*Permission.Builder builder2 = Permission.newBuilder();
            parser.merge(witness_permission, builder2);
            witnessPermission = builder2.build();*/

            Permission.Builder builder3 = Permission.newBuilder();
            for (int i = 0; i < active_permissions.length; i++) {
                parser.merge(active_permissions[i], builder3);
                activePermissions.add(builder3.build());
            }
...
//        builder.setWitness(witnessPermission);
        ...
        builder.addAllActives(activePermissions);
        builder.setOwnerAddress(owner);
        return builder.build();
    }
